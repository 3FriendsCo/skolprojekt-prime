...
namespace prime_error {
    namespace lexer {
        ...
        inline void note_error(clock_t& compiletime_start, std::ofstream* log_file, int error_code, int current_pos, int row, char current_char, const std::string line) {
            ...
            clock_t log_error_end = clock();
            double log_error_duration = (double)(log_error_end - compiletime_start) / CLOCKS_PER_SEC * 1000;
            
            *log_file << "[" << log_error_duration << "ms]:\n" << formatted_error << std::endl << std::endl << std::flush;
        }
    }
